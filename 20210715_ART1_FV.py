...
difference = [abs(x - 1) for x in sample_means]
plt.scatter(our_range, difference, marker=".", color="#000080", s=1)
plt.title("Differences of 5000 observations from Weibull(1,1)")
plt.xlabel("n")
...
difference = [abs(x - math.sqrt(math.pi/2)) for x in sample_means]
plt.scatter(our_range, difference, marker=".", color="#000080", s=1)
plt.title("Differences of 5000 observations from Rayleigh(1)")
plt.xlabel("n")
plt.ylabel("Differences")
plt.axhline(y=0, color='r', linestyle='-', linewidth=1)
